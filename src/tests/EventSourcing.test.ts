import { DeterministicEventLog } from '../engine/DeterministicEventLog';
...
describe('Deterministic Event Sourcing', () => {
  ...
  let log: DeterministicEventLog;
  let mockFetch: jest.Mock;

  beforeEach(() => {
    clock = new DeterministicTestClock(1000);
    log = new DeterministicEventLog(clock);
  ...
  const makeMockMarketContext = (seq: number = 0): MarketContext => ({
    timestamp: clock.now(),
    sequenceNumber: seq,
    symbol: 'BTCUSDT',
  ...
  test('fails if sequence number jumps or gets out of sync', () => {
    ...
    (log as any).sequenceCounter = 5; // Expected should be 1
  ...
  test('replayFrom performs strict chain verification', () => {
    ...
    expect(() => {
      (log as any).verifyEventChain(corruptedEvents);
    ...
    expect(() => {
      (log as any).verifyEventChain(corruptedHashEvents);
  ...
  test('triggers snapshot checkpoints on boundaries', () => {
    ...
    (log as any).triggerSnapshotCheckpoint(10000);
