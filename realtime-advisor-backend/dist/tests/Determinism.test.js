...
const AntigravityEngine_js_1 = require("../engine/AntigravityEngine.js");
...
const ReplayEngine_js_1 = require("../engine/ReplayEngine.js");
// Minimal raw context — matches exactly what background.js passes to evaluateMarket
const makeRawCtx = (overrides = {}) => ({
    timestamp: Date.now(),
    symbol: 'BTCUSDT',
...
describe('Determinism', () => {
    ...
    test('regime classification is deterministic', () => {
        ...
        expect(r1.context.regime).toBe(Types_js_1.MarketRegime.TRENDING);
        expect(r1.context.regime).toBe(r2.context.regime);
        ...
        expect(r3.context.regime).toBe(Types_js_1.MarketRegime.HIGH_VOLATILITY);
    ...
    test('constraint evaluation order matches registration order', () => {
        const engine = new AntigravityEngine_js_1.AntigravityEngine();
        const order = engine.constraintEngine.getEvaluationOrder();
    ...
    test('circuit breaker is deterministic with injected wall clock', () => {
        const engine = new AntigravityEngine_js_1.AntigravityEngine();
        const cb = engine.circuitBreakers;
...
describe('Bayesian Probability Engine', () => {
    test('starts at 50% with Beta(2,2) prior', () => {
        const engine = new AntigravityEngine_js_1.AntigravityEngine();
        const prob = engine.probEngine;
        const est = prob.getCalibratedBaseConfidence(Types_js_1.MarketRegime.COMPRESSION);
    ...
    test('posterior shifts toward wins', () => {
        ...
        for (let i = 0; i < 10; i++) {
            prob.recordTradeResult(Types_js_1.MarketRegime.COMPRESSION, true);
        }
        const est = prob.getCalibratedBaseConfidence(Types_js_1.MarketRegime.COMPRESSION);
    ...
    test('credible interval narrows as sample size grows', () => {
        const engine = new AntigravityEngine_js_1.AntigravityEngine();
        const prob = engine.probEngine;
        const before = prob.getCalibratedBaseConfidence(Types_js_1.MarketRegime.COMPRESSION);
        const widthBefore = before.credibleInterval95[1] - before.credibleInterval95[0];
        for (let i = 0; i < 30; i++) {
            prob.recordTradeResult(Types_js_1.MarketRegime.COMPRESSION, i % 2 === 0);
        }
        const after = prob.getCalibratedBaseConfidence(Types_js_1.MarketRegime.COMPRESSION);
    ...
    test('state serialises and deserialises correctly', () => {
        const engine = new AntigravityEngine_js_1.AntigravityEngine();
        const prob = engine.probEngine;
        prob.recordTradeResult(Types_js_1.MarketRegime.TRENDING, true);
        prob.recordTradeResult(Types_js_1.MarketRegime.TRENDING, false);
        prob.recordTradeResult(Types_js_1.MarketRegime.COMPRESSION, true);
        const serialised = prob.serializeState();
        const engine2 = new AntigravityEngine_js_1.AntigravityEngine();
        const prob2 = engine2.probEngine;
        prob2.deserializeState(serialised);
        const est1 = prob.getCalibratedBaseConfidence(Types_js_1.MarketRegime.TRENDING);
        const est2 = prob2.getCalibratedBaseConfidence(Types_js_1.MarketRegime.TRENDING);
...
describe('Memory Safety', () => {
    test('event log stays within bounds under high load', () => {
        const clock = new DeterministicClock_js_1.DeterministicTestClock(1000);
        EventSourcing_js_1.EventLog.resetInstance();
        const eventLog = EventSourcing_js_1.EventLog.getInstance(clock);
        const engine = new AntigravityEngine_js_1.AntigravityEngine();
...
describe('Replay Parity', () => {
    test('replaying a live recorded log slice yields bit-for-bit parity', () => {
        ...
        for (let i = 0; i < 50; i++) {
            clock.advance(1000);
            const isWindow = (i % 5 === 0) ? Types_js_1.MarketState.EXECUTION_WINDOW : Types_js_1.MarketState.ACCUMULATION;
            ...
            if (i === 10) {
                engine.probEngine.recordTradeResult(Types_js_1.MarketRegime.TRENDING, true);
            }
            else if (i === 20) {
                engine.probEngine.recordTradeResult(Types_js_1.MarketRegime.TRENDING, false);
            }
            else if (i === 30) {
                engine.probEngine.recordTradeResult(Types_js_1.MarketRegime.COMPRESSION, true);
            }
        }
        ...
        const summary = ReplayEngine_js_1.ReplayEngine.replay(recordedEvents);
