...
export function detectLiquiditySweep(
  ...
  wickRatio: number = 0.5
): { bullishSweep: boolean; bearishSweep: boolean; level: number } {
  const len = candles.length;
  if (len < lookbackPeriod) return { bullishSweep: false, bearishSweep: false, level: 0 };
...
export interface FVGRegistryItem {
  id: string;
  creationTime: number;
...
export function getSwingFractals(candles: Candle[]) {
...
export function updateLiquidityPools(
  candles: Candle[],
  dailyCandles: Candle[],
...
export function detectLiquiditySweeps(
  ...
  const len = candles.length;
  if (len < 2 || pools.length === 0) return null;
  const current = candles[len - 1];

  let bestSweep: SweepEvent | null = null;
...
export function calculateDisplacement(candles: Candle[], candleIndex: number): number {
  if (candleIndex < 20 || candleIndex >= candles.length) return 0;
...
export function updateFVGRegistry(
  candles: Candle[],
  registry: FVGRegistryItem[],
  currentTickPrice: number
): FVGRegistryItem[] {
  if (candles.length < 4) return registry;
  const len = candles.length;
  ...
  registry.forEach(f => {
    if (f.status === "MITIGATED") return;

    if (f.direction === "BULLISH") {
      let lowestPrice = currentPrice;
      for (let idx = len - 1; idx >= 0; idx--) {
        if (candles[idx].time < f.creationTime) break;
        if (candles[idx].low < lowestPrice) lowestPrice = candles[idx].low;
      }
      ...
      for (let idx = len - 1; idx >= 0; idx--) {
        if (candles[idx].time < f.creationTime) break;
        if (candles[idx].high > highestPrice) highestPrice = candles[idx].high;
      }
...
export function calculateDealingRange(
  ...
  if (direction === "LONG") {
    ...
    if (highestHigh === -Infinity) highestHigh = candles[len - 1].high;
    ...
    if (lowestLow === Infinity) lowestLow = candles[len - 1].low;
